...
Static personal website deployed on Railway using Docker + Node.js server.

## Local run

```bash
docker build -t lampros-site .
docker run --rm -p 8080:3000 lampros-site
```
...
5. Do not hardcode a port; Railway injects `PORT` automatically and `server.js` uses it.
...
3. Deployment logs contain successful Node startup log: "website running on port".
