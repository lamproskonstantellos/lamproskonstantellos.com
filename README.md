...
- Article pages include `Article` schema JSON-LD with author, dates, headline, image, full text, and (when set) `topics` as `about` entities. The home page includes `ProfilePage` / `Person` JSON-LD (with `worksFor` / `affiliation` / `alumniOf` from `site.config.js`); `/publications` carries a typed `ItemList` with a DOI-bearing node per publication.
